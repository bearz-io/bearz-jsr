...
test("cmd::simple inline test", skip(!WINDOWS), async () => {
    const cmd1 = await cmdScript("echo Hello, World!");
    equal(cmd1.text(), "Hello, World!\r\n");
    equal(0, cmd1.code);
});

test("cmd::simple inline test with options", skip(!WINDOWS), async () => {
    const cmd1 = await cmdScript("echo Hello, World!").run();
    equal(0, cmd1.code);
});

test("cmd:: multi-line inline test", skip(!WINDOWS), async () => {
...
test("cmd: simple file test", skip(!WINDOWS), async () => {
    await writeTextFile(
        "test.cmd",
        `
    @echo off
